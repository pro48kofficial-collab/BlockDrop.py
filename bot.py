...
async def main():
    if not BOT_TOKEN:
        raise RuntimeError("BOT_TOKEN не заданий. Додай BOT_TOKEN у Environment Variables на Render.")

    port = int(os.getenv("PORT", "10000"))
    render_url = os.getenv("RENDER_EXTERNAL_URL", "").rstrip("/")
    if not render_url:
        raise RuntimeError("RENDER_EXTERNAL_URL не знайдено. Для Render Web Service він додається автоматично.")
    ...
    print(f"[BlockDrop] Webhook: {webhook_url}")
